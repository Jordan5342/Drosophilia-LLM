...
def arxiv_search(action_input, max_papers = 5, folder_name = ".", **kwargs):
    ...
    return observation
def flybase_query(action_input, invalid_action_error, folder_name = ".", **kwargs):
    """Query FlyBase for Drosophila gene information"""
    try:
        query = parse_action_input(action_input, ["query"])[0].strip()
    except:
        return "FlyBase query failed due to parsing. Use format: gene_summary:Stat92E or drosophila_ortholog:JAK2"
    
    # Check if FlyBase tool is initialized
    current_history = kwargs.get('current_history')
    if not current_history or 'tool_instances' not in current_history or 'flybase' not in current_history['tool_instances']:
        return "Error: FlyBase tool not initialized. Run with --flybase flag."
    
    try:
        flybase_tool = current_history['tool_instances']['flybase']
        result = flybase_tool(query)
        return result
    except Exception as e:
        return f"FlyBase query error: {str(e)}"
...
def summarize_remaining_genes(all_genes, summary_size=20, bs=1000):
    ...
    for idx, b in tqdm(enumerate(blocks), total=len(blocks)):
        ...
        prompt = f"""
                ...
                """
        completion = complete_text(prompt, model="claude-3-5-sonnet-20240620", log_file=None)
...
ALL_TOOLS = {
    ...
    Action Input: {
        "gene_name": [a gene name],
            ...
            "FlyBase Query": {
        "description": "Query FlyBase database for Drosophila gene information. Use for: gene summaries, finding Drosophila orthologs of human genes, finding human orthologs of Drosophila genes, searching genes by phenotype, or getting RNAi phenotype data.",
        "usage": """
    Action: FlyBase Query
    Action Input: {
        "query": [Format: "query_type:parameter". Examples: "gene_summary:Stat92E", "drosophila_ortholog:JAK2", "human_ortholog:hop", "phenotype_search:immune", "rnai_phenotype:Rel"]
    }""".strip(),
        "return": "Gene information, ortholog mappings, or phenotype search results from FlyBase.",
        "function": flybase_query
